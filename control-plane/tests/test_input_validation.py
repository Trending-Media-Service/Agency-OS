...
async def test_tenant_input_length_validation(client):
    ...
    # but schema validation passes, so it will NOT be 422!
...
async def test_chat_input_length_validation(client):
    ...
    # Case 1: Valid input (within limits) -> should not be 422
    res_valid = await client.post(
        "/chat",
    ...
    # Case 2: Brand ID too long (101 chars) -> 422
    res_long_brand = await client.post(
        "/chat",
    ...
    # Case 3: Text too long (5001 chars) -> 422
    res_long_text = await client.post(
        "/chat",
        json={"brand_id": "b-brand", "text": "t" * 5001},
        headers=headers
    )
    assert res_long_text.status_code == 422
