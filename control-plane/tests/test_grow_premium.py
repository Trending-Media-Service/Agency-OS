...
@pytest.mark.asyncio
async def test_grow_premium_meridian_mmm_audit(grow_adapter, base_op):
    ...
    res = await grow_adapter.execute(op, "idem_mmm_audit")
    assert res.ok is True
    assert res.detail["storage_status"] == "ok"
    assert res.detail["incrementality_ratio"] == 0.184
    assert res.detail["roi_multiplier"] == 4.2
...
@pytest.mark.asyncio
async def test_grow_premium_meridian_mmm_audit_degraded_flow(grow_adapter, base_op, monkeypatch):
    ...
    res = await grow_adapter.execute(op, "idem_mmm_outage")
    assert res.ok is True
...
@pytest.mark.asyncio
async def test_grow_premium_ai_readiness_audit(grow_adapter, base_op):
    """Test that grow.ai_readiness.audit scans the ad campaign, scores AI readiness,
    and returns precise operator recommendations for failed checks.
    """
    op = replace(base_op, 
    ...
    res = await grow_adapter.execute(op, "idem_ai_audit")
    assert res.ok is True
    assert res.detail["campaign_id"] == "camp-test-ai-1"
    assert res.detail["score"] == 75.0  # 3 out of 4 checks passed
    assert res.detail["checks"]["smart_bidding_enabled"] is False
    assert len(res.detail["recommendations"]) == 1
    assert "Upgrade campaign bidding strategy to Target CPA or Target ROAS" in res.detail["recommendations"][0]
