...
from .models import Brand, OpRow, OpTrace, Tenant, TrustSnapshot, Connection, Order, Cadence
...
async def evaluate_trust(s: AsyncSession = Depends(get_worker_db)):
    ...
    # Structure: {provider: {campaign_id: {roas, op, current_budget_minor}}}
    ...
    # If Meta ROAS is significantly higher than Google ROAS, propose reallocation Saga
        ...
        if best_meta["roas"] >= 1.5 * worst_google["roas"] and worst_google["budget_minor"] > transfer_amount_minor:
            ...
            if not res_dup_saga.scalar_one_or_none():
                ...
                parent_saga = OpRow(
                    ...
                    idem_key=f"idem_saga_realloc_{saga_id}"
                )
                s.add(parent_saga)
...
async def process_cadences(s: AsyncSession = Depends(get_db)):
    ...
    # Query due cadences
    stmt = select(Cadence).where(Cadence.next_run <= now, Cadence.status.in_(["on_track", "due"]))
    res = await s.execute(stmt)
