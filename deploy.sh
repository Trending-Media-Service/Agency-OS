...
gcloud run services update "$BACKEND_SERVICE" \
  ...
  --set-env-vars="^###^ALLOWED_ORIGINS=$ALLOWED_ORIGINS###ENV=production###GCP_PROJECT=$GCP_PROJECT_ID###GCP_LOCATION=$REGION###AOS_STATE_BUCKET=aos-tfstate-tmg###OUTBOX_QUEUE_NAME=aos-outbox###APP_URL=https://agency-os-backend-730671240713.asia-south1.run.app" \
...
echo "Backend is now live at: $(gcloud run services describe "$BACKEND_SERVICE" --platform managed --region "$REGION" --format="value(status.url)" --project "$GCP_PROJECT_ID")"
