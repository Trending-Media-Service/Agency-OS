...
sed -i "s|_PROJECT:\s*\S+|_PROJECT: $PROJECT_ID|g" "$CLOUDBUILD_FILE"
sed -i "s|_REGION:\s*\S+|_REGION: $REGION|g" "$CLOUDBUILD_FILE"
sed -i "s|_REPO:\s*\S+|_REPO: wellness|g" "$CLOUDBUILD_FILE"
sed -i "s|_API_SERVICE:\s*\S+|_API_SERVICE: $API_SERVICE|g" "$CLOUDBUILD_FILE"
sed -i "s|_FRONTEND_SERVICE:\s*\S+|_FRONTEND_SERVICE: $FRONTEND_SERVICE|g" "$CLOUDBUILD_FILE"
sed -i "s|_API_URL:\s*\S+|_API_URL: $API_URL|g" "$CLOUDBUILD_FILE"
sed -i "s|_FRONTEND_ORIGIN:\s*\S+|_FRONTEND_ORIGIN: $FRONTEND_ORIGIN|g" "$CLOUDBUILD_FILE"
...
gcloud builds submit --config=cloudbuild.yaml --project="$PROJECT_ID" --substitutions=COMMIT_SHA=latest --quiet "$TEMP_DIR"
